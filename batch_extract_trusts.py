...
import pipeline_shared as shared
...
_load_dotenv_file = shared.load_dotenv_file
read_xlsx_rows = shared.read_xlsx_rows
normalize_company_number = shared.normalize_company_number
_ensure_parent = shared.ensure_parent
write_json = shared.write_json
_utc_now = shared.utc_now
_utc_now_precise = shared.utc_now_precise
_append_jsonl = shared.append_jsonl
_parse_fallback_models = shared.parse_fallback_models
...
def _install_companies_house_request_throttle(
    ...
    return shared.install_request_throttle(
        client=client,
...
_resolve_cached_pdf_for_company = shared.resolve_cached_pdf_for_company
_latest_full_accounts_document_id_from_filing_history = (
    shared.latest_full_accounts_document_id_from_filing_history
)
_extraction_types_for_schema_profile = shared.extraction_types_for_schema_profile
_derive_annual_report_from_component_sections = (
    shared.derive_annual_report_from_component_sections
)
_extract_with_model_fallback = shared.extract_with_model_fallback
...
def build_parser() -> argparse.ArgumentParser:
    ...
    shared.add_common_extraction_cli_args(
        parser,
        output_root_default=DEFAULT_OUTPUT_ROOT,
        output_root_help="Root directory to store PDFs and JSON outputs",
        db_help="SQLite file path (defaults to <output-root>/companies_house_extractions.db)",
        model_help="OpenRouter model (defaults to OPENROUTER_MODEL env var)",
    )
    return parser
